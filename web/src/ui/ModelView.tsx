  ...
  return (
    <div className="flex h-full flex-col bg-neutral-50 text-neutral-900">
      ...
      <main className="mx-auto w-full max-w-3xl flex-1 space-y-4 p-6">
        <h2 className="text-sm font-medium text-neutral-700">Storeys</h2>
        {model.isLoading && <p className="text-sm text-neutral-500">Loading…</p>}
        {model.error && <p className="text-sm text-red-600">Could not load model.</p>}
    ...
    <li className="px-4 py-3">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-neutral-500">elev {storey.elevation.toFixed(2)} m</p>
        </div>
        <div className="flex items-center gap-3">
      ...
      {status === 'preparing' && (
        <div className="mt-2">
          ...
          <p className="mt-1 text-xs text-neutral-500">{progress?.stage ?? 'starting…'}</p>
        </div>
      )}
      {status === 'error' && floor.data && (
        <p className="mt-1 text-xs text-red-600">prep failed — retry</p>
      )}
    </li>
  )
}
