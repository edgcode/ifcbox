...
python route.py model.ifc --floor 1 --list-spaces
python route.py model.ifc --floor 1 --list-terminals

# route between two world points, open the PyVista viewer
python route.py model.ifc --floor 1 --start-xyz 5.3,41.8,6.9 --end-xyz 9.3,42.4,6.9

# route between rooms (IfcSpace GlobalIds), export only, write debug PNGs + shell/walls/rooms
python route.py model.ifc --floor 1 --start-room <space_id> --end-room <space_id> --no-view --debug --shell

# batch demo: Flur → adjacent rooms per apartment (Steiner trees)
python demo_routes.py model.ifc --floor 1 --no-view
```
...
- **Which floor?** Pick a residential/occupied storey with `IfcSpace`s (rooms) — the demo model's floor 1 (`OKFF OG1`) is a good example. (Raw-deck levels named `OKRD` / `UKRD` — top / bottom of structural slab in German naming — are filtered out of the storeys list automatically.)
